...
use candle_core::{op::Op, DType, Device, Module, Result, Tensor, D};
...
impl Attention {
    ...
    fn forward(
        ...
        seqlen_offset: usize,
        cache_key_values: Option<bool>,
    ) -> Result<Tensor> {
        let cache_key_values = cache_key_values.or(Some(false));
...
impl DecoderLayer {
    ...
    fn forward(
        ...
        let xs = self
            .self_attn
            .forward(&xs, attention_mask, seqlen_offset, cache_key_values)?;
        let xs = (xs + residual)?;
        let residual = &xs;
